...
public class QuantumEntanglement {
    ...
    private static Pair<Long, Integer> calculateLowestQEScore(
        ...
        // Have to validate that the remaining sets can be created based off of what we have created so far!
    ...
    private static boolean canCreateRemainingEqualSets(
        ...
        // We have to check both conditions of adding the currentIndex value and not adding its value to the total
        // sum. For the one that we add its element to the sum, we also add the index to the used indices.
        // In both cases, we move to the next index.
        return canCreateRemainingEqualSets(
                items,
                targetSum,
                requiredPartitions,
