...
import com.capital7software.aoc2015.lib.util.Triple;
 ...
 */
public record AStartAndWizards(SpellShop shop, Mage mage, SwordsAndStuff.Player boss) {
    ...
    public record Spell(SpellType type, String name, int cost, int damage, int armor, int health, int mana, int turns) {
    }

    public record SpellShop(List<Spell> spells) {
    }

    public record Mage(String name, int hitPoints, int mana, List<Spell> spells) {
     ...
     */
    public static AStartAndWizards buildSimulator(List<String> input) {
        ...
        return new AStartAndWizards(new SpellShop(spells), mage, boss);
    }
