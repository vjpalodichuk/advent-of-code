...
import com.capital7software.aoc.lib.grid.BathroomCode
...
class Day02 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val answer = calculateSimpleBathroomCode(input)
    val end = Instant.now()

    log.info("{} is the simple code to the bathroom!", answer)
    logTimings(log, start, end)
  }

  override fun runPart2(input: List<String>) {
    val start = Instant.now()
    val answer = calculateComplexBathroomCode(input)
    val end = Instant.now()

    log.info("{} is the complex code to the bathroom!", answer)
    logTimings(log, start, end)
  }
   ...
   */
  fun calculateSimpleBathroomCode(input: List<String>) : String {
    val instance = BathroomCode.buildBathroomCode(input)

    return instance.findSimpleBathroomCode()
  }
   ...
   * @return The bathroom code from the specified instructions.
   */
  fun calculateComplexBathroomCode(input: List<String>) : String {
    val instance = BathroomCode.buildBathroomCode(input)

    return instance.findComplexBathroomCode()
  }
}
