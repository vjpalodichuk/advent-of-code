...
import com.capital7software.aoc.lib.crypt.SecureRooms
...
class Day04 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val answer = calculateSumOfRealSectorIds(input)
    val end = Instant.now()

    log.info("{} is the sum of the sector IDs!", answer)
    logTimings(log, start, end)
  }

  override fun runPart2(input: List<String>) {
    val start = Instant.now()
    val answer = calculateSectorIdOfRoomWhereObjectsAreStored(input)
    val end = Instant.now()

    log.info("{} is the sector ID where North Pole objects are stored!", answer)
    logTimings(log, start, end)
  }

  /**
   * Calculates and returns the sum of the sector IDs of the real rooms.
   *
   * @param input The [List] of [String] which are the rooms to parse.
   * @return The sum of the sector IDs of the real rooms.
   */
  fun calculateSumOfRealSectorIds(input: List<String>): Long {
    val instance = SecureRooms.buildSecureRooms(input)
    return instance.sunOfRealRoomSectorIds()
  }

  /**
   * Calculates and returns the sector ID of the real room where North Pole
   * objects are stored.
   *
   * @param input The [List] of [String] which are the rooms to parse.
   * @return The sector ID of the real room where North Pole
   * objects are stored.
   */
  fun calculateSectorIdOfRoomWhereObjectsAreStored(input: List<String>): Long {
    val instance = SecureRooms.buildSecureRooms(input)
    val rooms = instance.decryptRealRooms()
    val room = rooms.firstOrNull { it.first().equals("northpole object storage") }
    return if (room == null) {
      -1L
    } else {
      room.second()
