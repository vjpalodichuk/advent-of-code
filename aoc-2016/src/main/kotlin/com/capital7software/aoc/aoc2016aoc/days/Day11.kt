 ...
 *
 * >What is the **minimum number of steps** required to bring all the objects,
...
class Day11 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val answer = minimumStepsToSolve(input)
    val end = Instant.now()

    log.info("$answer is the minimum number of steps to bring all objects to the fourth floor!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: List<String>) {
    val start = Instant.now()
    val answer = minimumStepsToSolveWithAddedPieces(input)
    val end = Instant.now()
  ...
  /**
   * Runs the [GeneratorsAndChips] game using the specified [List] of [String] instructions and
   * returns the minimum number of steps to solve the game.
   *
   ...
   */
  @SuppressFBWarnings
  fun minimumStepsToSolve(input: List<String>): Int {
    val game = GeneratorsAndChips.buildGeneratorAndChips(input)

    return game.solve().first
  }

  /**
   * Runs the [GeneratorsAndChips] game using the specified [List] of [String] instructions and
   * returns the minimum number of steps to solve the game. Adds four pieces to the game before
   * trying to solve it.
   *
   * @param input The [List] of [String] to parse into [GeneratorsAndChips.Floor].
   * @return The minimum number of steps to solve the game.
  ...
  fun minimumStepsToSolveWithAddedPieces(input: List<String>): Int {
    ...
    game.addMicrochip("elerium")
    game.addGenerator("dilithium")
    game.addMicrochip("dilithium")
