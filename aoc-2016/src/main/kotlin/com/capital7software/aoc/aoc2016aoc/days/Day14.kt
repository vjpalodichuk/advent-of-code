...
class Day14 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val answer = indexOfNthKeyGenerated(salt, limit)
    val end = Instant.now()

    log.info("$answer is the index of the $limit key!")
  ...
  override fun runPart2(input: List<String>) {
    ...
    val limit = 64
    val start = Instant.now()
    val answer = indexOfNthKeyGenerated(salt, limit, true)
    val end = Instant.now()

    log.info("$answer is the index of the $limit stretched key!")
    logTimings(log, start, end)
  }

  /**
   * Calculates and returns the index of the last key generated.
   *
   * @param salt The secret to add to generate the MD5 hashes.
   * @param limit The number of keys to generate.
   * @return The index of the last key generated.
   */
  @SuppressFBWarnings
  fun indexOfNthKeyGenerated(
      salt: String,
      limit: Int,
      stretched: Boolean = false
  ): Int {
    return Md5Fun.generateOneTimePadKeys(salt, limit, stretched = stretched).last().second()
  }
}
