...
class Day15 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val answer = timeToPushTheButton(input)
    val end = Instant.now()

    log.info("$answer is the time to push the button to get the capsule!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: List<String>) {
    val start = Instant.now()
    val answer = timeToPushTheButtonWithAdditionalDisc(input)
    val end = Instant.now()
   ...
   */
  @SuppressFBWarnings
  fun timeToPushTheButton(input: List<String>): Int {
  ...
  /**
   * Calculates and returns the time to push the button of the sculpture so that the capsule
   * it releases goes through all the discs in the sculpture. This method adds a
   * disc to the sculpture before calculating the time to push the button.
   *
   * @param input The [List] of [String] to parse into a [DiscSculpture]
   */
