...
import com.capital7software.aoc.lib.graph.path.SecureVault
 ...
 * --- Part Two ---
 *
 * You're curious how robust this security solution really is, and so you decide to find
 * longer and longer paths which still provide access to the vault. You remember that paths
 * always end the first time they reach the bottom-right room (that is, they can never pass
 * through it, only end in it).
 *
 * For example:
 *
...
class Day17 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val answer = findShortestPath(input.first())
    val end = Instant.now()

    log.info("$answer is the shortest path through the vault!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: List<String>) {
    val start = Instant.now()
    val answer = findLongestPathLength(input.first())
    val end = Instant.now()

    log.info("$answer is the longest path length through the vault!")
    logTimings(log, start, end)
  }
   ...
   */
  @SuppressFBWarnings
  fun findShortestPath(input: String): String {
    val instance = SecureVault(input, 4, 4)
    return instance.findShortestPath().second
  }
