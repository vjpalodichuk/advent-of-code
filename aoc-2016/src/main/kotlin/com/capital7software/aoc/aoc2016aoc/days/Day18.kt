...
import com.capital7software.aoc.lib.grid.TrapRoom
...
class Day18 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val answer = findNumberOfSafeTiles(input.first(), rows)
    val end = Instant.now()

    log.info("$answer is the number of safe tiles!")
  ...
  /**
   * Calculates and returns the number of safe rows.
   *
   * @param input The starting row of the map.
   * @return The shortest path.
   */
  @SuppressFBWarnings
  fun findNumberOfSafeTiles(input: String, rows: Int): Int {
    val instance = TrapRoom(input, rows)
    return instance.countSafeTiles()
  }
}
