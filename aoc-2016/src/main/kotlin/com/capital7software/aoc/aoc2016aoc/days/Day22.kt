...
import com.capital7software.aoc.lib.graph.path.GridComputing
 ...
 *
 * Your goal is to gain access to the data which begins in the node with y=0 and the
 * **highest x** (that is, the node in the top-right corner).
 *
 ...
 * The node you can access directly, node-x0-y0, is almost full. The node containing
 * the data you want to access, node-x2-y0 (because it has y=0 and the highest x value),
 * contains 6 terabytes of data - enough to fit on your node, if only you could make
 * enough space to move it there.
 *
 * Fortunately, node-x1-y1 looks like it has enough free space to enable you to move
 * some of this data around. In fact, it seems like all of the nodes have enough space
 * to hold any node's data (except node-x0-y2, which is much larger, very full, and not
 * moving any time soon). So, initially, the grid's capacities and connections look like this:
 *
 * ```
 * ( 8T/10T) --  7T/ 9T -- [ 6T/10T]
 *     |           |           |
 ...
 * around. Let's draw these nodes as .. The exceptions are the empty node, which we'll
 * draw as _, and the very large, very full node, which we'll draw as #. Let's also
 ...
 */
class Day22 : AdventOfCodeSolution {
  companion object {
    private val log: Logger = LoggerFactory.getLogger(Day22::class.java)
  }
  ...
  override fun runPart1(input: List<String>) {
    ...
    val answer = viablePairs(input)
    val end = Instant.now()

    log.info("$answer is the number of viable node-pairs!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: List<String>) {
    val start = Instant.now()
    val answer = fewestStepsToMoveGoalData(input)
    val end = Instant.now()

    log.info("$answer is the fewest number of steps to move goal data to destination!")
    logTimings(log, start, end)
  }

  /**
   * Calculates and returns the total number of node pairs where the first can move its data to
   * the second.
   *
   * @param input The list of output from the df -h command.
   * @return The total number of valid pairs.
   */
  @SuppressFBWarnings
  fun viablePairs(input: List<String>): Int {
    val instance = GridComputing(input)
    val pairs = instance.viablePairs()
    return pairs.size
  }

  /**
   * Returns the fewest steps to move the data from the goal to the source node.
   *
   * @param input The list of output from the df -h command.
   * @return The fewest steps to move the data from the goal to the source node.
   */
  fun fewestStepsToMoveGoalData(input: List<String>): Int {
    val instance = GridComputing(input)
    val start = instance[0, 0]
    val x = instance.columns - 1
    val goal = instance[x, 0]

    return instance.shortestPathToMoveData(start, goal).first
  }
}
