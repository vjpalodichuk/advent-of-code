...
import com.capital7software.aoc.lib.computer.JumpToExit
...
class Day05 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: MutableList<String>) {
    ...
    val answer = stepsToExitLoop(input)
    val end = Instant.now()

    log.info("$answer is the number steps to take to exit the loop!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: MutableList<String>) {
    val start = Instant.now()
    val answer = stepsToExitLoop(input, true)
    val end = Instant.now()
  ...
  /**
   * Returns the number of steps required to exit the specified loop of jump offsets.
   *
   * @param input The [List] of [String] integers to process.
   * @param newRules If set to true, then the new rules to decrease by 1 if offset greater
   * than or equal to three will be in effect.
