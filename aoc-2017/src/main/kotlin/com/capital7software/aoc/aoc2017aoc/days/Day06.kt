...
import com.capital7software.aoc.lib.computer.MemoryReallocation
...
class Day06 : AdventOfCodeSolution {
  ...
  override fun getDefaultInputFilename(): String = "inputs/input_day_06-01.txt"
  ...
  /**
   * Returns the number of redistribution cycles that must be completed to detect the
   * infinite loop. If [lengthCount] is true, instead it returns the length of the
   * infinite loop cycle.
   *
   * @param input The string representation of the banks and their current values.
   * @param lengthCount If true, then this method returns the length of the infinite loop
   * cycle.
   * @return The number of redistribution cycles that must be completed to detect the
   * infinite loop. If [lengthCount] is true, instead it returns the length of the
   * infinite loop cycle.
   */
  fun cyclesRequiredToDetectLoop(input: String, lengthCount: Boolean = false): Int {
    val instance = MemoryReallocation(input)

    return if (lengthCount) {
      instance.cyclesInLoop.first
    } else {
      instance.cycleCount.first
    }
  }
}
