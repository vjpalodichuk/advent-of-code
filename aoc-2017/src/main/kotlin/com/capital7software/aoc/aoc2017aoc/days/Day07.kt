...
import com.capital7software.aoc.lib.computer.RecursiveCircus
...
class Day07 : AdventOfCodeSolution {
  ...
  override fun getDefaultInputFilename(): String = "inputs/input_day_07-01.txt"

  override fun runPart1(input: MutableList<String>) {
    val start = Instant.now()
    val answer = programAtBottomOfTower(input)
    val end = Instant.now()

    log.info("$answer is the program at the bottom of the tower!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: MutableList<String>) {
    val start = Instant.now()
    val answer = unbalancedProgramWeight(input)
    val end = Instant.now()

    log.info("$answer is the weight the unbalanced program should be!")
    logTimings(log, start, end)
  }

  /**
   * Returns the name of the program that is at the bottom of the tower.
   *
   * @param input The [List] of [String] which defines the programs, their weights, and which
   * programs are being held up by other programs.
   * @return The name of the program that is at the bottom of the tower.
   */
  fun programAtBottomOfTower(input: List<String>): String {
    val instance = RecursiveCircus(input)
   ...
   * @return The name of the program that is at the bottom of the tower.
   */
  fun unbalancedProgramWeight(input: List<String>): Int {
    val instance = RecursiveCircus(input)

    return instance.getUnbalancedProgram().second
  }
}
