 ...
 * Hash of your puzzle input?** Ignore any leading or trailing whitespace you might encounter.
 *
...
class Day10 : AdventOfCodeSolution {
  ...
  override fun getDefaultInputFilename(): String = "inputs/input_day_10-01.txt"

  override fun runPart1(input: MutableList<String>) {
    val start = Instant.now()
    val answer = getCheckValue(input.first())
    val end = Instant.now()

    log.info("$answer is the check value from the hash!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: MutableList<String>) {
    val start = Instant.now()
    val answer = getKnotHash(input.first())
    val end = Instant.now()

    log.info("$answer is the knot hash of the input!")
    logTimings(log, start, end)
  }

  /**
   * Returns the check value of the hashed input.
   *
   * @param input The [String] to parse into lengths.
   * @param listSize The size of the list to hash.
   * @return The check value of the hashed input.
   */
  fun getCheckValue(input: String, listSize: Int = 256): Int {
    val instance = KnotHash(input, listSize)
    return instance.check
  }

  /**
   * Returns the knot hash of the input. Accurate only if listSize is 256.
   *
   * @param input The [String] to parse into lengths.
   * @param listSize The size of the list to hash.
   * @return The knot hash of the input. Accurate only if listSize is 256.
   */
  fun getKnotHash(input: String, listSize: Int = 256): String {
    val instance = KnotHash(input, listSize)
