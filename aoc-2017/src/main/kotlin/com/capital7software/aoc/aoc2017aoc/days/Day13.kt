...
import com.capital7software.aoc.lib.math.PacketScanner
...
class Day13 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: MutableList<String>) {
    ...
    val answer = takeTrip(input)
    val end = Instant.now()

    log.info("${answer.second} is the severity starting at time ${answer.first}!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: MutableList<String>) {
    val start = Instant.now()
    val answer = takeTrip(input, true)
    val end = Instant.now()
  ...
  /**
   * Returns the total severity of the trip through the firewall.
   *
   * @param input The [List] of [String] that defines the layer depths and scanner ranges of
   * the firewall.
   * @return The total severity of the trip through the firewall.
   */
  fun takeTrip(input: List<String>, catchFree: Boolean = false): Pair<Int, Int> {
    val instance = PacketScanner(input, catchFree)
