...
import com.capital7software.aoc.lib.grid.DiskDefragmentation
...
class Day14 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: MutableList<String>) {
    ...
    val answer = defragmentation(input.first())
    val end = Instant.now()

    log.info("${answer.first} is the number of used squares after defragmentation!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: MutableList<String>) {
    val start = Instant.now()
    val answer = diskRegions(input.first())
    val end = Instant.now()

    log.info("${answer} is the number regions!")
    logTimings(log, start, end)
  }

  /**
   * Returns the results of the [DiskDefragmentation] process. The first element is the
   * number of used squares and the second element is the number of free squares.
   *
   * @param input The [String] key that is used for generating the Knot Hashes.
   * @return A [Pair] where the first element is the number of used squares and the second
   * element is the number of free squares.
   */
  fun defragmentation(input: String): Pair<Int, Int> {
    val instance = DiskDefragmentation(input)
    return instance.results
  }

  /**
   * Returns the number of regions after running the [DiskDefragmentation] process.
   *
   * @param input The [String] key that is used for generating the Knot Hashes.
   * @return The number of regions after running the [DiskDefragmentation] process.
   */
  fun diskRegions(input: String): Int {
    val instance = DiskDefragmentation(input)
