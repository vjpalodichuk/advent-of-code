...
import com.capital7software.aoc.lib.analysis.DuelingGenerators
...
class Day15 : AdventOfCodeSolution {
  ...
  override fun getDefaultInputFilename(): String = "inputs/input_day_15-01.txt"

  override fun runPart1(input: MutableList<String>) {
    val start = Instant.now()
    val answer = generatorMatchCount(input)
    val end = Instant.now()

    log.info("${answer} is the number of times in 40 million iterations the lower 16-bits matched!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: MutableList<String>) {
    val start = Instant.now()
    val answer = generatorMatchCount(input, true)
    val end = Instant.now()

    log.info("${answer} is the number of times in 5 million pairs the lower 16-bits matched!")
    logTimings(log, start, end)
  }

  /**
   * Returns the number of times that both generator's lower 16-bits matched.
   *
   * @param input The starting values for the generators.
   * @param newLogic If true, then the generators will act independently and only produce
   * 5 million values to consider using the new logic instead of 40 million.
   * @return The number of times that both generator's lower 16-bits matched.
   */
  fun generatorMatchCount(input: List<String>, newLogic: Boolean = false): Int {
    val instance = DuelingGenerators(input, newLogic)
