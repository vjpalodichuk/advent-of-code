 ...
 * each named with a single letter and that can each hold a single integer. You suppose
 ...
 * two instructions to another jump, which jumps again to the rcv, which ultimately
 * triggers the recover operation.
 *
 ...
 * program ID of the other program (program 0 receives 1; program 1 receives 0) and stores
 * it in c. Each program now sees a different value in its own copy of register c.
 *
 ...
 *
 * It should be noted that it would be equally valid for the programs to run at different
 * speeds; for example, program 0 might have sent all three values and then stopped at
 * the first rcv before program 1 executed even its first instruction.
 *
...
class Day18 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: MutableList<String>) {
    ...
    val answer = recoverFrequency(input)
    val end = Instant.now()

    log.info("$answer is the value of the recovered frequency!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: MutableList<String>) {
    val start = Instant.now()
    val answer = getMessageSentCount(input)
    val end = Instant.now()

    log.info("$answer is the number of times program 1 sent a message to program 2!")
    logTimings(log, start, end)
  }

  /**
   * Returns the value of the recovered frequency.
   *
   * @param input The raw [List] of [String] assembly instructions.
   * @return The value of the recovered frequency.
   */
  fun recoverFrequency(input: List<String>): Long {
    var soundFrequency: Long = Long.MIN_VALUE
    val instance = SmallComputer.buildSmallComputer(
        input,
        useSound = true,
        soundHandler = {
          value: Long ->
          if (soundFrequency == Long.MIN_VALUE) {
            soundFrequency = value
          }
        }
    )
    while (soundFrequency == Long.MIN_VALUE) {
  ...
  /**
   * Returns the number of times program 1 sent program 2 a message.
   *
   * @param input The raw [List] of [String] assembly instructions.
   * @return The number of times program 1 sent program 2 a message.
   */
  fun getMessageSentCount(input: List<String>): Int {
    val instance = SmallComputer.buildSmallComputer(
        input,
        useSound = false
    )
    val contexts = instance.run()
    return contexts.second?.sendCount() ?: -1
  }
}
