...
import com.capital7software.aoc.lib.analysis.SporificaVirus
 ...
 *
 * Given your actual map, after 10000000 bursts of activity, **how many bursts cause a
...
class Day22 : AdventOfCodeSolution {
  ...
  override fun getDefaultInputFilename(): String = "inputs/input_day_22-01.txt"

  override fun runPart1(input: MutableList<String>) {
    val bursts = 10_000
    val start = Instant.now()
    val answer = runBursts(input, bursts)
    val end = Instant.now()

    log.info("${answer.first} is number of bursts that caused an infection out of $bursts bursts!")
  ...
  override fun runPart2(input: MutableList<String>) {
    ...
    val start = Instant.now()
    val answer = runBursts(input, bursts, true)
    val end = Instant.now()
   ...
   * @param bursts The number of iterations to perform.
   * @param newLogic If true, then the new virus logic is used.
   * @return The number of pixels that are in the on position of the [FractalArt] after the
   * specified number of iterations are performed.
   */
  @SuppressFBWarnings
  fun runBursts(input: List<String>, bursts: Int, newLogic: Boolean = false): Pair<Int, Int> {
    val instance = SporificaVirus(input)
    instance.reset()
    val answer = instance.update(bursts, newLogic)
    return answer
  }
}
