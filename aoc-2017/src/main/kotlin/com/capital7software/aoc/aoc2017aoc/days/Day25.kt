...
import com.capital7software.aoc.lib.computer.TuringMachine
...
class Day25 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: MutableList<String>) {
    ...
    val answer = getDiagnosticChecksum(input)
    val end = Instant.now()

    log.info("$answer is the diagnostic checksum value!")
    logTimings(log, start, end)
  }

  override fun runPart2(input: MutableList<String>) {
    val start = Instant.now()
    val answer = "The printer has been rebooted!"
    val end = Instant.now()
  ...
  /**
   * Executes the [TuringMachine] until it returns its diagnostic checksum value.
   *
   * @param input The [TuringMachine] blueprints.
   * @return The diagnostic checksum value.
   */
  @SuppressFBWarnings
  fun getDiagnosticChecksum(input: List<String>): Int {
    val instance = TuringMachine(input)
