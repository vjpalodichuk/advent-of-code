...
import com.capital7software.aoc.lib.analysis.ReposeRecord
...
class Day04 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = getGuardChecksum(input)
    }
    log.info("$answer is the product of the guard id and minute where they slept the most!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Int
    val elapsed = measureNanoTime {
      answer = getGuardAlternateChecksum(input)
    }
    log.info("$answer is the product of the guard id and minute where they slept the most!")
    logTimings(log, elapsed)
  }
  ...
  /**
   * Returns the product of the guard ID that sleeps the most and the minute
   * that guard slept during the most.
   *
   * @param input The list of overnight shift changes and falling asleep / waking up times.
   * @return The product of the guard ID that sleeps the most and the minute
   * that guard slept during the most.
   */
  @SuppressFBWarnings
  fun getGuardChecksum(input: List<String>): Int {
    return ReposeRecord(input).guardChecksum()
  }

  /**
   * Returns the product of the guard ID that sleeps the most during any particular minute
