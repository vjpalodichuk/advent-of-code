 ...
 * That must be where the sand comes from! Best of all, the boat races are starting in just
 * a few minutes.
 *
 * <p><br>
...
public class Day06 implements AdventOfCodeSolution {
   ...
   * (raceTime - holdTime)<br> This can be rearranged as:<br> d = rh - h²; 0 = rh -h² - d;
   * 0 = -h² + rh - d<br> Now all we do is change the = to &#62; to switch this to a quadratic
   * inequality:<br> -h² + rh - d &#62; 0<br> And now we can use the quadratic formula to find
   * our min and max times that satisfy the inequality and then the difference between them
   * plus 1 will give us the total number of winning hold times!
   *
   * <p><br>
   ...
   *
   * <p><br>
   * So now we can use the quadratic equation to set up the inequality in terms of the holdTime
   * as follows:<br> (-b - √(b²-4c))/(2) &#62; a &#62; (-b + √(b²-4c))/(2)<br>
   * We can multiply everything by -1 and substitute in our variables to get the following:<br>
   * raceTime - √(raceTime² - (4 * distance))/(2) &#62; holdTime &#62; raceTime +
   * √(raceTime² - (4 * distance))/(2)<br>
   *
   * <p><br>
   * Since we will get Doubles as a result, we take the floor of the upper bound and the ceiling
   * of the lower bound and add one to the difference between the two to get our answer!!
   *
   * @param input The race record times and distances.
  ...
  /**
   * For every millisecond the boat button is held, it will travel an additional 1
   * millimeter per millisecond. This is a brute force iterative implementation of the solution.
   *
   * @param input The race record times and distances.
   * @return The number of different hold / charge times that result in a
   *     distance greater than the specified distance.
