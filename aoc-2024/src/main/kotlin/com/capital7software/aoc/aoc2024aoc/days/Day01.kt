...
import com.capital7software.aoc.lib.analysis.HistorianHysteria
...
class Day01 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = calculateTotalDistance(input)
    }
    log.info("$answer is the resulting total distance!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = calculateSimilarityScore(input)
    }
    log.info("$answer is the resulting similarity score!")
    logTimings(log, elapsed)
  }

  /**
   * Returns the result of the total distance between each location ID pairs.
   *
   * @param input The list of location IDs.
   * @return The result of calculating the total distance between each location ID pair.
   */
  @SuppressFBWarnings
  fun calculateTotalDistance(input: List<String>): Long {
    return HistorianHysteria(input).calculateTotalDistance()
  }

  /**
   * Returns the result of calculating the similarity score of the location ID pairs.
   *
   * @param input The list of location IDs.
