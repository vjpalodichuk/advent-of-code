...
import com.capital7software.aoc.lib.game.GuardGallivant
...
class Day06 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = distinctNumberOfGuardPositions(input)
    }
    log.info("$answer is the number of distinct guard positions before leaving the area!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Int
    val elapsed = measureNanoTime {
      answer = numberOfPossibleObstructionPositions(input)
    }
    log.info("$answer is the number of possible positions for the new obstruction!")
    logTimings(log, elapsed)
  }
   ...
   */
  @SuppressFBWarnings
  fun distinctNumberOfGuardPositions(input: List<String>): Int {
    return GuardGallivant(input).countDistinctGuardPositionsByPoint()
  }
