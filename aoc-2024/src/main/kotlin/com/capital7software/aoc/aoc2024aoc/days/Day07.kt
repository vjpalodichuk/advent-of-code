...
class Day07 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = sumOfPossiblyTrueTestValuesAddAndMultiplyOnly(input)
    }
    log.info("$answer is total calibration result of the possibly valid test equations! (+ *)")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = sumOfPossiblyTrueTestValuesAllOperators(input)
    }
    log.info("$answer is total calibration result of the possibly valid test equations! (+ * ||)")
    logTimings(log, elapsed)
  }
  ...
  /**
   * Returns the sum of the possibly true test values. Uses only the ADD and MULTIPLY operators.
   *
   * @param input The list of strings that form the test values and equations.
   * @return The sum of the possibly true test values. Uses only the ADD and MULTIPLY operators.
   */
  @SuppressFBWarnings
  fun sumOfPossiblyTrueTestValuesAddAndMultiplyOnly(input: List<String>): Long {
    return BridgeRepair(input).possiblyTrueTestValues(addAndMulOnly = true)
        .keys
        .sumOf { it.expected }
  }

  /**
   * Returns the sum of the possibly true test values. Uses all available operators
   *
   * @param input The list of strings that form the test values and equations.
