...
import com.capital7software.aoc.lib.analysis.DiskFragmenter
...
class Day09 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = filesystemDefragmentation(input)
    }
    log.info("$answer is the filesystem checksum after defragmentation!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = fastFilesystemDefragmentation(input)
    }
    log.info("$answer is the filesystem checksum after fast defragmentation!")
    logTimings(log, elapsed)
  }

  /**
   * Returns the filesystem checksum after defragmentation.
   *
   * @param input The [String] that represents the current drive-map.
   * @return The filesystem checksum after defragmentation.
   */
  @SuppressFBWarnings
  fun filesystemDefragmentation(input: List<String>): Long {
    val result = DiskFragmenter(input.first()).compactDefragmentation()

    return result
  }

  /**
   * Returns the filesystem checksum after fast defragmentation.
   *
   * @param input The [String] that represents the current drive-map.
