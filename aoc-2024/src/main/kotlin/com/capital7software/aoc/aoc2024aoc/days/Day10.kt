...
import com.capital7software.aoc.lib.graph.path.HoofIt
...
class Day10 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = sumAllTrailheadScores(input)
    }
    log.info("$answer is the sum of all trailhead scores!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = sumAllTrailheadRatings(input)
    }
    log.info("$answer is the sum of all trailhead ratings!")
    logTimings(log, elapsed)
  }

  /**
   * Returns the sum of the score of every trailhead in the specified map.
   *
   * @param input The [String] that represents the trail map.
   * @return The sum of the score of every trailhead in the specified map.
   */
  @SuppressFBWarnings
  fun sumAllTrailheadScores(input: List<String>): Long {
    val result = HoofIt(input).findAll().sumOf { it.score.toLong() }

    return result
  }

  /**
   * Returns the sum of the rating of every trailhead in the specified map.
   *
   * @param input The [String] that represents the trail map.
