...
class Day11 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = numberOfStonesAfterTwentyFiveBlinks(input.first())
    }
    log.info("$answer is the number of stones after 25 blinks!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = numberOfStonesAfterSeventyFiveBlinks(input.first())
    }
    log.info("$answer is the number of stones after 75 blinks!")
    logTimings(log, elapsed)
  }

  /**
   * Returns the number of stones after 25 blinks.
   *
   * @param input The [String] the initial stone arrangements.
   * @return The number of stones after 25 blinks.
   */
  @SuppressFBWarnings
  fun numberOfStonesAfterTwentyFiveBlinks(input: String): Int {
    val result = PlutonianPebbles(input).blink(25).size

    return result
  }

  /**
   * Returns the number of stones after 75 blinks.
   *
   * @param input The [String] the initial stone arrangements.
