...
import com.capital7software.aoc.lib.grid.GardenGroups
...
class Day12 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = totalPriceByPerimeterOfAllFencing(input)
    }
    log.info("$answer is the total price, using the perimeter method, of fencing all of the garden regions!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = totalPriceBySidesOfAllFencing(input)
    }
   ...
   */
  @SuppressFBWarnings
  fun totalPriceByPerimeterOfAllFencing(input: List<String>): Long {
    val groups = GardenGroups.create(input)
    val result = groups.totalByPerimeter.toLong()
