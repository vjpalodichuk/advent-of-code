...
import com.capital7software.aoc.lib.math.ClawContraption
 ...
 * [arcade](https://en.wikipedia.org/wiki/Amusement_arcade)! Maybe you can win some prizes
 * from the [claw](https://en.wikipedia.org/wiki/Claw_machine) machines?
 *
 * The claw machines here are a little unusual. Instead of a joystick or directional buttons
 * to control the claw, these machines have two buttons labeled A and B. Worse, you can't
 * just put in a token and play; it costs **3 tokens** to push the A button and **1 token**
 * to push the B button.
 *
 * With a little experimentation, you figure out that each machine's buttons are configured to
 * move the claw a specific amount to the **right** (along the X axis) and a specific amount
 ...
 * the ```B``` button ```40```0 times. This would line up the claw along the ```X``` axis
 ...
 * button ```38``` times and the ```B``` button ```86``` times. Doing this would cost a total
 * of **```200```** tokens.
 *
 * So, the most prizes you could possibly win is two; the minimum tokens you would have to
 * spend to win all (two) prizes is **```480```**.
 *
 ...
 *
 * Add `10000000000000` to the `X` and `Y` position of every prize. After making this change,
...
class Day13 : AdventOfCodeSolution {
  ...
  override fun getDefaultInputFilename(): String = "inputs/input_day_13-01.txt"

  override fun runPart1(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = fewestTokensToWinAllPrizes(input)
    }
    log.info("$answer is the fewest number of tokens needed to win all prizes!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = fewestTokensToWinAllPrizesWithOffset(input)
    }
    log.info("$answer is the total price, using the sides method, of fencing all of the garden regions!")
    logTimings(log, elapsed)
  }

  /**
   * Returns the total price of fencing all the garden regions using the Perimeter method.
   *
   * @param input The [List] of [String]s that represent the garden layout.
   * @return The total price of fencing all the garden regions using the Perimeter method.
   */
  @SuppressFBWarnings
  fun fewestTokensToWinAllPrizes(input: List<String>): Long {
    val contraption = ClawContraption.load(input)
    val result = contraption.cost

    return result
  }

  /**
   * Returns the total price of fencing all the garden regions using the Sides method.
   *
   * @param input The [List] of [String]s that represent the garden layout.
   * @return The total price of fencing all the garden regions using the Sides method.
   */
  fun fewestTokensToWinAllPrizesWithOffset(input: List<String>): Long {
    val contraption = ClawContraption.load(input, 10_000_000_000_000)
    val result = contraption.cost
