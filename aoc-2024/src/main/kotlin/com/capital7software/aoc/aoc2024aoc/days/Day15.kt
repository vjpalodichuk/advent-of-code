...
import com.capital7software.aoc.lib.grid.WarehouseWoes
 ...
 * for right) that the robot will attempt to make, in order. (The moves form a single giant
 * sequence; they are broken into multiple lines just to make copy-pasting easier. Newlines within
 ...
 * locations of the boxes. The **GPS coordinate** of a box is equal to 100 times its distance from
 * the top edge of the map plus its distance from the left edge of the map. (This process does not
 ...
 * finishes moving. In the larger example, the sum of all boxes' GPS coordinates is **`10092`**. In
 * the smaller example, the sum is **`2028`**.
 *
...
class Day15 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    log.info("$answer is the sum of the Goods Positioning System coordinates from all boxes!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = gpsSum(input, true)
    }
    log.info("$answer is the sum of the Goods Positioning System coordinates from all large boxes!")
    logTimings(log, elapsed)
  }

  /**
   * Returns the GPS Sum of all boxes in the warehouse.
   *
   * @param input The [List] of [String]s that represent the objects in the warehouse.
   * @return The GPS Sum of all boxes in the warehouse.
   */
  @SuppressFBWarnings
  fun gpsSum(input: List<String>, large: Boolean = false): Long {
    val woes = WarehouseWoes.load(input, large)
    woes.execute()
    val result = woes.gpsSum
