...
import com.capital7software.aoc.lib.string.LinenLayout
 ...
 * [hot springs](https://adventofcode.com/2023/day/12) on Gear Island! Very
 * [suspiciously](https://www.youtube.com/watch?v=ekL881PJMjI), absolutely nothing goes wrong as
 * they begin their careful search of the vast field of helixes.
 *
 * Could this **finally** be your chance to visit the 
 * [onsen](https://en.wikipedia.org/wiki/Onsen) next door? Only one way to find out.
 *
 ...
 * stripe (`wr`), and so on.
 *
 * After the blank line, the remaining lines each describe a design the onsen would like to be 
 * able to display. In this example, the first design (`brwrr`) indicates that the onsen would like
 * to be able to display a black stripe, a red stripe, a white stripe, and then two red stripes, in
 * that order.
 *
...
class Day19 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = possibleDesignCount(input)
    }
    log.info("$answer designs are possible!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = totalArrangementCount(input)
    }
    log.info("$answer is the total number of possible towel arrangements!")
    logTimings(log, elapsed)
  }
   ...
   */
  fun possibleDesignCount(input: List<String>): Long {
    return LinenLayout.parse(input).possibleDesignCount()
  }

  /**
   * Counts all possible towel arrangements across all desired designs.
   *
   * @param input The puzzle input.
   * @return The total number of possible arrangements.
   */
  fun totalArrangementCount(input: List<String>): Long {
    return LinenLayout.parse(input).totalArrangementCount()
  }
}
