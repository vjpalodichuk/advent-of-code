 ...
 * **race**. Apparently, you've arrived just in time for the frequently-held **race condition**
 * festival!
 *
 * The race takes place on a particularly long and twisting code path; programs compete to see who
 * can finish in the **fewest picoseconds**. The winner even gets their very own
 ...
 * again; otherwise, it will receive a
 * [segmentation fault](https://en.wikipedia.org/wiki/Segmentation_fault) and get disqualified.
 *
 * So, a program could complete the course in 72 picoseconds (saving **12 picoseconds**) by 
 * cheating for the two moves marked `1` and `2`:
 *
 * ```
 * ###############
 * #...#...12....#
 ...
 * ```
 * 
 * Or, a program could complete the course in 64 picoseconds (saving **20 picoseconds**) by 
 * cheating for the two moves marked `1` and `2`:
 *
 * ```
 * ###############
 ...
 * cheat that instead lasts at most **20 picoseconds**.
 *
...
class Day20 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = countCheatsThatSaveAtLeast(input, minimumSavings = 100, maxCheatDuration = 2)
    }
    log.info("$answer cheats save at least 100 picoseconds!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = countCheatsThatSaveAtLeast(input, minimumSavings = 100, maxCheatDuration = 20)
    }
    log.info("$answer cheats save at least 100 picoseconds with the longer cheat duration!")
    logTimings(log, elapsed)
  }

  /**
   * Counts how many distinct cheats save at least [minimumSavings] picoseconds.
   *
   * @param input The puzzle input.
   * @param minimumSavings The minimum number of picoseconds a cheat must save.
   * @param maxCheatDuration The maximum duration of the cheat.
  ...
  fun countCheatsThatSaveAtLeast(
    ...
    return RaceCondition.parse(input)
        .countCheatsThatSaveAtLeast(minimumSavings, maxCheatDuration)
  }
}
