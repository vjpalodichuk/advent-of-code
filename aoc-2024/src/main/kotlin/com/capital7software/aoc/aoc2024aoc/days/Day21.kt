 ...
 * The Historians begin to panic: someone from their search party is **missing**. A quick life-form
 * scan by the ship's computer reveals that when the missing Historian teleported, he arrived in
 ...
 * keypad that would cause the robot to type `029A`: `<A^A>^^AvvvA`, `<A^A^>^AvvvA`, and
 * `<A^A^^>AvvvA`.
 *
 ...
 * the upper right corner. After that, a **second, different** directional keypad remote control
 * is used to control this robot (in the same way as the first robot, except that this one is
 ...
 * this robot to tell the second robot to tell the first robot to eventually type `029A` on the
 * door. One such sequence is
 ...
 * particular, if a robot arm is ever aimed at a **gap** where no button is present on the keypad,
 * even for an instant, the robot will **panic** unrecoverably. So, don't do that. All robots will
 ...
 * Historian is trapped in the area containing a **giant electromagnet** or **molten lava**.
 * You'll need to make sure that for each of the five codes, you find the **shortest sequence** of
 ...
 * keypad in order to cause the code to be typed on the numeric keypad; for `029A`, this would be
 * `68`.
 * - The **numeric part of the code** (ignoring leading zeroes); for `029A`, this would be `29`.
 *
 * In the above example, complexity of the five codes can be found by calculating
 * `68 * 29, 60 * 980`, `68 * 179`, `64 * 456`, and `64 * 379`. Adding these together produces
 * **`126384`**.
 *
 ...
 * Due to a variety of hazards, robots are once again dispatched, forming another chain of remote
 ...
 * typing on a directional keypad which controls a robot which is typing on a directional
 * keypad... and so on, ending with the robot which is typing on the numeric keypad.
 *
...
class Day21 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = sumOfComplexities(input, directionalRobots = 2)
    }
    log.info("$answer is the sum of complexities with 2 directional robots!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = sumOfComplexities(input, directionalRobots = 25)
    }
    log.info("$answer is the sum of complexities with 25 directional robots!")
    logTimings(log, elapsed)
  }
   ...
   * @return The sum of complexities.
   */
  fun sumOfComplexities(
      input: List<String>,
      directionalRobots: Int
  ): Long {
    return KeypadConundrum.parse(input)
        .sumOfComplexities(directionalRobots)
  }
}
