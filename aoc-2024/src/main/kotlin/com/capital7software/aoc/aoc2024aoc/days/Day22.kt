 ...
 *
 * As you're all teleported deep into the jungle, a [monkey](https://adventofcode.com/2022/day/11)
 * steals The Historians' device! You'll need to get it back while The Historians are looking for
 * the Chief.
 *
 ...
 * [pseudorandom](https://en.wikipedia.org/wiki/Pseudorandom_number_generator)! If you know the
 ...
 * [bitwise XOR](https://en.wikipedia.org/wiki/Bitwise_operation#XOR) of the given value and the
 * secret number. Then, the secret number becomes the result of that operation. (If the secret
 * number is `42` and you were to **mix** `15` into the secret number, the secret number would
 * become `37`.)
 * - To **prune** the secret number, calculate the value of the secret number
 * [modulo](https://en.wikipedia.org/wiki/Modulo) `16777216`. Then, the secret number becomes the
 * result of that operation. (If the secret number is `100000000` and you were to **prune** the
 ...
 *
 * For each buyer, simulate the creation of 2000 new secret numbers. **What is the sum of the
 * 2000th secret number generated by each buyer?**
 *
 ...
 * about a new hiding spot. However, you still don't speak 
 * [monkey](https://adventofcode.com/2022/day/21), so you can't negotiate with the buyers directly.
 * The Historian speaks a little, but not enough to negotiate; instead, he can ask another monkey 
 ...
 * is going to generate `2000` secret numbers after their initial secret number, so, for each buyer,
 ...
 *
 * - For the buyer with an initial secret number of `1`, changes `-2,1,-1,3` first occur when the 
 * price is **`7`**.
 * - For the buyer with initial secret `2`, changes `-2,1,-1,3` first occur when the price is 
 * **`7`**.
 * - For the buyer with initial secret `3`, the change sequence `-2,1,-1,3` **does not occur** in 
 * the first 2000 changes.
 ...
 * changes in every buyer's future prices, you get the most bananas in total. **What is the most 
 * bananas you can get?**
 *
...
class Day22 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = sumOfNthSecret(input)
    }
    log.info("$answer is the sum of each buyer's 2000th generated secret number!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: Long
    val elapsed = measureNanoTime {
      answer = mostBananas(input)
    }
    log.info("$answer is the most bananas obtainable!")
    logTimings(log, elapsed)
  }

  /**
   * Returns the sum of each buyer's nth generated secret.
   *
   * @param input The puzzle input.
   * @param generations The number of secret numbers to generate for each buyer.
   * @return The sum of each buyer's nth generated secret.
   */
  fun sumOfNthSecret(
      input: List<String>,
      generations: Int = 2000
  ): Long {
    return MonkeyMarket.parse(input)
        .sumOfNthSecret(generations)
  }

    /**
   * Returns the most bananas obtainable with a single four-change sequence.
   *
   * @param input The puzzle input.
   * @param generations The number of secret numbers to generate for each buyer.
   * @return The most bananas obtainable.
   */
  fun mostBananas(
      input: List<String>,
      generations: Int = 2000
