...
import com.capital7software.aoc.lib.graph.LanParty
 ...
 * [LAN party](https://en.wikipedia.org/wiki/LAN_party) scheduled for today! Maybe you can find it;
 * you connect to a nearby [datalink port](https://adventofcode.com/2016/day/9) and download a map
 ...
 * of connected computers. Start by looking for **sets of three computers** where each computer in
 * the set is connected to the other two computers.
 *
 ...
 *
 * Your puzzle answer was **`am,bv,ea,gh,is,iy,ml,nj,nl,no,om,tj,yv`**.
 */
class Day23 : AdventOfCodeSolution {
  ...
  override fun runPart1(input: List<String>) {
    ...
    val elapsed = measureNanoTime {
      answer = countHistorianCandidateTriples(input)
    }
    log.info("$answer sets of three interconnected computers contain a computer starting with t!")
  ...
  override fun runPart2(input: List<String>) {
    var answer: String
    val elapsed = measureNanoTime {
      answer = lanPartyPassword(input)
    }
    log.info("$answer is the LAN party password!")
    logTimings(log, elapsed)
  }

  /**
   * Returns the number of fully-connected groups of three computers where at least one computer
   * starts with `t`.
   *
   * @param input The puzzle input.
   * @return The number of matching groups.
   */
  fun countHistorianCandidateTriples(input: List<String>): Int {
    return LanParty.parse(input)
        .countInterconnectedTriples("t")
  }

  /**
   * Returns the LAN party password.
   *
   * The password is the largest fully-connected group of computers, sorted alphabetically and joined
   * by commas.
   *
   * @param input The puzzle input.
   * @return The LAN party password.
   */
  fun lanPartyPassword(input: List<String>): String {
    return LanParty.parse(input)
        .password()
  }
}
