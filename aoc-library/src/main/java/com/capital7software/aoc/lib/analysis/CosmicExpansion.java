 ...
 * For each pair, find any shortest path between the two galaxies using only steps that move up, down,
 * left, or right exactly one . or # at a time. (The shortest path between two galaxies is allowed
 ...
 * 9 (the eight locations marked # plus the step onto galaxy 9 itself). Here are some other examples
...
public class CosmicExpansion {
    ...
    private static class Universe {
        ...
        private static void processCharacter(char item, int row, int column, @NotNull Universe universe) {
        ...
        private void addGalaxy(@NotNull Point2D<Integer> point) {
            ...
            columnGalaxyCountMap.put(point.x(), columnGalaxyCountMap.computeIfAbsent(point.x(), it -> 0) + 1);
         ...
         */
        private void calculateAndStoreDistance(@NotNull Galaxy first, @NotNull Galaxy second, long gapFactor) {
    ...
    /**
     * Builds a new CosmicExpansion by constructing a new Universe and populating it with the Galaxies in
     * the specified List of Strings.
     *
     ...
     * The Optimized parameter, if true, will use a time and space optimized algorithm for calculating
     * the sum of the shortest paths between all pairs.
     *
     * @param gapFactor The gap factor to use when calculating the gap space.
     * @param optimized If true, will use the time and space optimized algorithm for calculating distances.
