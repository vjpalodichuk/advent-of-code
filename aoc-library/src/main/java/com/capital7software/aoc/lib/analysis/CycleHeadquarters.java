 ...
 * memory for that input. Then, if it remembers high pulses for all inputs, it sends a low pulse;
 ...
 *
 * <p><br>
 * Here at Desert Machine CycleHeadquarters, there is a module with a single button on it
 ...
 * a single-input conjunction module (inv), a multi-input conjunction module (con), and an
 * untyped module named output (for testing purposes). The multi-input conjunction module con
 * watches the two flip-flop modules and, if they're both on, sends a low pulse to the
 * output module.
 *
 * <p><br>
 ...
 * and 4 high pulses are sent. So, after pushing the button 1000 times, 8000 low pulses and 4000
...
public class CycleHeadquarters {
    ...
    public Map.Entry<Module, Pulse> processNextPulse() {
      ...
      if (entry == null) {
        throw new RuntimeException(
            "processNextPulse called with no pulses waiting to be processed!");
      }
  ...
  private record ModuleConfiguration(Broadcaster broadcaster, @NotNull List<Module> modules) {
    ...
    public static ModuleConfiguration build(List<String> input) {
      ...
      var modulesById = modules
          .keySet()
          .stream()
          .collect(Collectors.toMap(Module::getId, it -> it));
    ...
    public void processPulses() {
      ...
      if (!modulesWithPulsesPending.isEmpty()) {
        throw new RuntimeException(
            "Processing failed as there are still modules with pending messages!");
      }
    }
  ...
  /**
   * Builds and returns a new CycleHeadquarters instance loaded with the specified
  ...
  /**
   * Returns the minimum number of button presses needed in order to send a low pulse to the
   * specified module. If maxButtonPresses is reached before a low pulse is sent to the specified
   * module then -1 is returned.
   *
   * @param id                  The ID of the module we want to send a low pulse to.
   * @param requiredSourceCount If the specified ID has more than one source, then how many of
   *                            them have to send a low pulse to the destination?
   * @param maxButtonPresses    The maximum number of button presses to perform.
   * @return The minimum number of button presses needed to send a low pulse to the specified
   *     module. Or -1 if the maximum number of button presses was reached before a low
   *     pulse would be sent to the specified module.
   */
  public long buttonPressesNeededToSendLowPulseToModule(
      @NotNull String id,
    ...
    if (broadcaster == null) {
      throw new RuntimeException("Unable to find broadcaster module with id: "
                                     + ModuleConfiguration.BROADCASTER_ID);
    }
    ...
    if (watchList.size() < requiredSourceCount) {
      throw new RuntimeException(
          "The destination doesn't have the required number of sources! "
              + "Required source count: " + requiredSourceCount + " destination source count: "
              + watchList.size()
      );
    }
  ...
  private Map<Module, Set<Module>> getWatchList(Module module) {
    ...
    module.getSourceModules()
        .forEach(source ->
                     targetSets.computeIfAbsent(source, it -> new HashSet<>(it.getSourceModules()))
        );
