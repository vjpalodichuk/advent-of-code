...
public record GearRatios() {
     ...
     * @param symbolOffset     The offset in the schematic where the symbol it was adjacent to was found.
    ...
    public @NotNull List<PartNumber> findAllPartNumbers(@NotNull List<String> schematic) {
        ...
        return partNumbers.stream().filter(NumberEntry::hasPartNumber).map(NumberEntry::getPartNumber).toList();
    }
    ...
    public @NotNull List<Long> findAllGearRatios(@NotNull List<PartNumber> partNumbers) {
        ...
        return potentialGears
                ...
                .map(it -> (long) potentialGears.get(it).getFirst().id() * potentialGears.get(it).get(1).id())
                .toList();
    }
