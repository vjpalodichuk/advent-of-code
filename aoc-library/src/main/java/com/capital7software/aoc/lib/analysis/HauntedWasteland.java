 ...
 * follow all the paths at the same time until they all simultaneously end up at nodes
 * that end with Z.
 *
 * <p><br>
 * For example:<br><br>
...
public record HauntedWasteland(
  ...
  public long traverse(@NotNull String startNode, @NotNull Predicate<String> atEndNode) {
    ...
    if (currentNode != null) {
      ...
      var message = "Something went wrong after taking {} steps when starting at {} as "
          + "we didn't end at the correct node!";
      LOGGER.warn(
          message,
          stepCount, startNode);
    }
  ...
  private @NotNull BinaryTreeNode<String> takeStep(
      char nextStep,
      @NotNull BinaryTreeNode<String> currentNode
  ) {
    if (nextStep == LEFT) {
      ...
      throw new RuntimeException(
          "Unable to take the next step due to an unknown Direction: " + nextStep);
    }
  }
}