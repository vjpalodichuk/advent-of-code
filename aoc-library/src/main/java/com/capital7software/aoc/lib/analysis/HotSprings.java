...
public class HotSprings {
  ...
  private record SpringRecord(List<Spring> springs, List<Integer> damagedSequence) {
    ...
    /**
     * Returns the total number of valid sequences that are possible that satisfy the
     * damagedSequence of this record. A valid sequence is one where the Unknown Springs
     * are replaced with either Operational or Damaged Springs and the damagedSequence
     * is satisfied.
     *
     * @return The total number of valid sequences that are possible that satisfy the
     *     damagedSequence of this record.
     */
    ...
    private long validSequences(HashMap<SpringRecord, Long> cache) {
      ...
      // If there are no Springs left to process, then we check if we have any damaged
      // sequences left as well. If we don't have any damaged sequences left to check,
      // then we return 1 as the sequence is valid, else 0.
      if (springs.isEmpty()) {
      ...
      // If we have some Springs left, but we don't have any damaged sequences left
      // to check, then if the remaining springs are all operational the sequence
      // is valid, and we return 1, else 0.
      if (damagedSequence.isEmpty()) {
      ...
      // If the next spring could be Damaged we have to check that the conditions to ensure
      // that the damaged sequence is met. The conditions are:
      ...
      // - After the damage sequence, there are no Springs left to process, the sequence is
      // ended by an Operational Spring or possibly Operational Spring.
      // If these conditions are met, we can move on to the next damage sequence and process
  ...
  /**
   * Builds and returns a new HotSprings from the List of Strings. The Strings represent the
   * spring records to parse. If unfold is true, then each spring record is replaced with five
   * copies of itself!
   *
