 ...
 * You must have enough mana to cast a spell, and its cost is immediately deducted when
 * you cast it.<br>
 *
 * <p><br>
 ...
 *
 * <p><br>
 * Effects all work the same way. Effects apply at the start of both the mage's turns and the
 ...
 * this decreases the timer to zero, the effect ends. You cannot cast a spell that would start an
 * effect which is already active. However, effects can be started on the same turn they end.<br>
 *
 * <p><br>
 * For example, suppose the mage has 10 hit points and 250 mana, and that the boss has
 ...
 */
public record StarAndWizards(SpellShop shop, Mage mage, SwordsAndStuff.Player boss) {
  ...
  private static class GameState {
    private record EffectsState(
        int playerHitPoints, int playerMana, int bossHitPoints, int armor,
    ...
    public void bossTurn() {
      var updates = applyEffects(
          new EffectsState(playerHitPoints, playerMana, bossHitPoints, 0, effects)
      );
    ...
    public int hashCode() {
      return Objects.hash(
          playerHitPoints, playerMana, bossHitPoints, bossDamage, spentMana, effects, hardMode
      );
    }
   ...
   */
  public static StarAndWizards buildSimulator(List<String> input) {
    ...
    return new StarAndWizards(new SpellShop(spells), mage, boss);
  }
  ...
  private static Pair<Mage, SwordsAndStuff.Player> parseMage(String line) {
    ...
    if ("Boss".equals(name)) {
      ...
      boss.add(new SwordsAndStuff.Item(
          SwordsAndStuff.ItemType.WEAPON,
  ...
  /**
   * Uses A* Shortest Path to find the minimum amount of Mana needed in order
   * to defeat the boss.<br>
   * The primary key for selecting the next state to explore is the least amount of
   * Mana that has currently been spent and if two states share the same key, then the second
   * key is used. The second key is the order in which the states were created and so the earliest
   * created state would be selected in cases of a tie.<br>
   *
   * @param hard If true, the mage will lose one point of damage before each turn.
   * @return A Pair where the first property is the amount of mana spent to win the game and
   *     the second property is a copy of the Mage and the spells it cast.
   */
  ...
  private GameState findMinimumManaSpend(GameState start) {
    var gameStates = new PriorityQueue<>(
        50,
        Comparator
