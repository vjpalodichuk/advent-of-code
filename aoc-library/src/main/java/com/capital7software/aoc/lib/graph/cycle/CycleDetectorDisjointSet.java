 ...
 *
 * <p><br>
 * When this detector is created a map between the supplied nodeIds and their associated
 * DisjointSet instance is created. Each instance of DisjointSet is initialized with the parent
 * set to the corresponding nodeId. When detect is called, the parents of the two nodes are found.
 * If they have the same parent then true is returned as a cycle would be formed. If they have two
 * different parents, then the two are merged in to the same set and if they had the same rank,
 * the rank of the new set is increased.
 *
 * <p><br>
 * Inspired by the Path Compression and Union Rank algorithms used in Kruskal's MST
 * algorithm found at:
...
public class CycleDetectorDisjointSet implements CycleDetector {
   ...
   *
   * @param nodeIds The Collection of nodeIds that are in the Graph we are doing cycle detection
   *                for.
   */
  ...
  /**
   * Creates a Map between the nodeIds and their DisjointSet instance. The DisjointSet for each
   * nodeId is initialized with the parent set to the nodeId that instance is associated with and
   * a default rank of 1.
   *
   * @param nodeIds THe Collection of nodeIds that are in the Graph we are doing cycle detection
   *                for.
  ...
  /**
   * Uses Path Compression for finding and updating the parents for the Nodes when searching for
   * them. Uses Union Rank when determining which set to merge into the other.
   *
   * <p><br>
   * When detect is called, the parents of both node IDs are updated while being retrieved. Path
   * Compression is used during the find operation. This is done  If they already have the same
   * parent, then a cycle would be formed if an edge would be added between them. If they have
   * different parents, the two sets are merged using a Union Rank algorithm. The root parent with
   * the higher rank will be the parent of the merged set. The merged set's rank is only increased
   * if both nodes were at the same rank prior to the merge operation.
   *
   * <p><br>
   ...
   * @return True if a cycle would be formed by adding an Edge from the source Vertex to the
   *     target Vertex.
   */
  @Override
  ...
  /**
   * Finds and returns the nodeId of the root parent node.
   *
