...
public class ClumsyCrucible {
    ...
    private static class Crucible {
        ...
        private record PathCost(@NotNull Path path, long cost, long distanceToGoal) implements Comparable<PathCost> {
        ...
        public static Grid2D<Tile> parseGrid(List<String> input) {
            ...
            return new Grid2D<>(columns.get(), rows.get(), tiles.toArray(new Tile[columns.get() * rows.get()]));
        }
        ...
        private static boolean parseRow(String line, ArrayList<Tile> rowTiles, int row) {
            ...
            for (int column = 0; column < chars.length; column++) {
                rowTiles.add(new Tile(Integer.parseInt(String.valueOf(chars[column])), new Point2D<>(column, row)));
            }
        ...
        private Path takeStepForward(@NotNull Path path) {
            ...
            var newPoint = grid.pointInDirection(point, direction);
    ...
    /**
     * Builds and returns a new ClumsyCrucible where the crucible starts in the upper-left corner and moves to the
     * lower=right corner of the grid. The minimumSteps parameter controls the minimum number of steps in the same
     * direction that the crucible must travel before it can turn left or right. The maximumSteps parameter
     * controls the maximum number of steps in the same direction that the crucible can travel before it must
     * turn left or right.
     *
    ...
    /**
     * Calculates the path for this crucible to take that results in the minimum loss of heat and returns that value.
     *
