...
public class GardenSteps {
  ...
  private record Garden(Grid2D<GardenTile> grid, Point2D<Integer> initialPosition) {
    ...
    /**
     * Take a walk through this Garden starting at the specified position and taking the
     * specified number of steps from that position.
     *
     * <p><br>
     * If the specified position is not in this Garden a RuntimeException is thrown.
     *
     * <p><br>
     * If the specified position is not a walkable tile then a RuntimeException is thrown.
     *
     * <p><br>
     * If the specified number of steps is less than 0 then a RuntimeException is thrown.
     *
     ...
     * @return The total number of plots in this garden that can be reached from the
     *     specified starting position and walking the specified number of steps.
     */
    private long walk(Point2D<Integer> startingPosition, long steps, boolean virtual) {
      if (!grid.isOnGrid(startingPosition)) {
        throw new RuntimeException("The startingPosition is not in this Garden! "
                                       + startingPosition);
      }
      if (steps < 0) {
      ...
      for (int stepsTaken = 1; stepsTaken <= stepsToTake; stepsTaken++) {
        ...
        for (var tile : previousStepsNeighbors) {
          for (var neighbor : getWalkableNeighbors(tile, virtual)) {
            ...
            if (stepsTaken % 2 == 0 || stepsTaken == stepsToTake
                && !reachedPlots.contains(neighbor)) {
   ...
   * @return The number of garden plots that can be reached by walking the specified
   *     number of steps.
   */
