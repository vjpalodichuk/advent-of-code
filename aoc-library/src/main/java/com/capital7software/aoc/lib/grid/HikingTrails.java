...
//            List.copyOf(trailSegments);
    ...
    /**
     * Builds and returns a new HikingTrails instance. The instance is populated with the available trails that
     * lead to the exit along with the distance of each trail. If ignoreSlopes is true, then slopes are ignored
     * when building the hiking trails. If virtualGrid is true, then the grid is treated as if it were virtual.
     *
     * @param input        The List of Strings to parse into HikingTrails.
     * @param ignoreSlopes If true, then the slopes on the trails are ignored when building the trails.
     ...
     */
    public static HikingTrails buildHikingTrails(List<String> input, boolean ignoreSlopes, boolean virtualGrid) {
