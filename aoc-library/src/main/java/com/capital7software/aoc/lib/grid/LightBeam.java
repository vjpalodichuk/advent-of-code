...
public class LightBeam {
    ...
    private record Beam(Grid2D<Tile> grid, Point2D<Integer> startPoint, Direction startHeading) {
        ...
        public static Beam buildBeam(List<String> input) {
            ...
            return Beam.create(
                    new Grid2D<>(columns.get(), rows.get(), tiles.toArray(new Tile[columns.get() * rows.get()]))
            );
        }
        ...
        private List<? extends Map.Entry<Tile, Direction>> getNext(Tile tile, Direction heading) {
            ...
            directions.forEach(direction -> {
                var nextPoint = grid.pointInDirection(tile.point, direction);
        ...
        public int maxEnergized() {
            ...
            try (var pool = new ForkJoinPool(32)) {
                var tasks = new ArrayList<ForkJoinTask<Integer>>((int) (4 * (grid().size())));
        ...
        private int energize(int column, int row, Direction heading, Map<Tile, Set<Direction>> visited) {
    ...
    /**
     * Builds and returns a new LightBeam loaded with the layout from the specified List of Strings to parse.
     *
