...
public class PipeMaze {
    ...
    private static class MazeTile implements Comparable<MazeTile> {
        ...
        public boolean connectsNorthTo(MazeTile other) {
            return connectsNorth() && other.connectsSouth() && getY() == other.getY() + 1 && getX() == other.getX();
        }

        public boolean connectsEastTo(MazeTile other) {
            return connectsEast() && other.connectsWest() && getY() == other.getY() && getX() == other.getX() - 1;
        }

        public boolean connectsSouthTo(MazeTile other) {
            return connectsSouth() && other.connectsNorth() && getY() == other.getY() - 1 && getX() == other.getX();
        }

        public boolean connectsWestTo(MazeTile other) {
            return connectsWest() && other.connectsEast() && getY() == other.getY() && getX() == other.getX() + 1;
        }
    ...
    private static class StartMazeTile extends MazeTile {
        ...
        public static StartMazeTile fromDirections(HashSet<Direction> directions, Point2D<Integer> point) {
    ...
    public static PipeMaze buildPipeMaze(List<String> input) {
        ...
        var grid = new Grid2D<>(mazeSize.first(), mazeSize.second(), maze.toArray(new MazeTile[mazeTotalSize]));
    ...
    private static MazeTile determineStartingTileType(MazeTile startTile, Grid2D<MazeTile> grid) {
        ...
        if (startTile.getY() > 0 && grid.get(grid.pointInDirection(startPoint, Direction.NORTH)).connectsSouth()) {
            directions.add(Direction.NORTH);
        }
        if (startTile.getX() > 0 && grid.get(grid.pointInDirection(startPoint, Direction.WEST)).connectsEast()) {
            directions.add(Direction.WEST);
        }
        if (startTile.getY() < grid.rows() - 1 &&
                grid.get(grid.pointInDirection(startPoint, Direction.SOUTH)).connectsNorth()) {
            directions.add(Direction.SOUTH);
        }
        if (startTile.getX() < grid.columns() - 1 &&
                grid.get(grid.pointInDirection(startPoint, Direction.EAST)).connectsWest()) {
    ...
    /**
     * Calculates and returns the number of tiles enclosed by the specified loop. The loop to pass to this method
     * can be obtained by calling calculateDistances.
     *
