 ...
 * A, K, Q, J, T, 9, 8, 7, 6, 5, 4, 3, or 2. The relative strength of each card follows
 ...
 * five hands in this example, the strongest hand will have rank 5 and its bid will be
 * multiplied by 5.
 *
 * <p><br>
...
public class CamelCards {
  ...
  public enum HandType {
    ...
    /**
     * Returns the HandType given the specified List of Cards. If joker is set to true,
     * then Jacks are filtered out in favor of Jokers.
     *
    ...
    public static HandType calculateHandType(@NotNull List<Card> cards, boolean joker) {
      ...
      if (joker && cardMap.containsKey(Card.JOKER)) {
        // If we are playing with Jokers, then we remove the Jokers from the map and add
        // their count to the highest frequency card in the map!
        ...
        for (var entry : cardMap.entrySet()) {
          ...
          if (count > maxCount
              || (count == maxCount && entry.getKey().getValue() > maxCard.getValue())) {
            maxCount = count;
    ...
    /**
     * Builds and returns a Map for the List of specified Cards where the key is the Card
     * and the value is the number of times that Card appears in the specified List of Cards.
     *
     ...
     *     and the value is how many times that frequency occurs in the specified Map.
     */
    public static @NotNull Map<Integer, Integer> buildFrequencyMap(
        @NotNull Map<Card, Integer> cardMap
    ) {
  ...
  public static class Hand implements Comparable<Hand> {
     ...
     * CCCCC 111<br>
     * 32T3K 765<br>
     * Jokers replace Jacks if joker is true. Returns the Hand parsed from the Cards and
     * bid String.
     *
     * @param handWithBid The String with the Cards and bid.
    ...
    /**
     * Parses the String of cards and returns a List of Cards. If joker is true, Jokers
     * replace Jacks.
  ...
  /**
   * Parses the specified Strings Hands and returns a CamelCard instance loaded with the
   * parsed Hands. If joker is true, then Jacks are replaced with Jokers.
   *
