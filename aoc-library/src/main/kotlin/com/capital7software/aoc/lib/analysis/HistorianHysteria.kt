 ...
 *
 * @param input The [String] [List] of location ID pairs.
 */
class HistorianHysteria @SuppressFBWarnings constructor(input: List<String>) {
  private val leftQueue: PriorityQueue<Long> = PriorityQueue()
  private val rightQueue: PriorityQueue<Long> = PriorityQueue()

  private companion object {
    private val EXTRACT_REGEX: Regex = "\\s*(?<left>\\d+)\\s+(?<right>\\d+)".toRegex()
  ...
  init {
      input.map {
        val match = EXTRACT_REGEX.find(it)
        check(match != null) { "Unable to parse $it." }
        leftQueue.add(match.groups[LEFT]!!.value.toLong())
        rightQueue.add(match.groups[RIGHT]!!.value.toLong())
      }
  }
  ...
  fun calculateSimilarityScore(): Long {
    ...
    val occurrences = rightQueue.groupingBy { it }.eachCount()

    leftQueue.forEach { num -> occurrences[num]?.let { sum += (num * it) } }
    return sum
  }
}