...
class GuardGallivant(input: List<String>) {
    ...
    private fun getGuardPositions(
        ...
        val visited = pathSoFar.toMutableList()
        val stepSet = hashSetOf<GuardStep>().apply { addAll(visited) }
        var onGrid = true
        var added = true

        if (visited.isNotEmpty()) {
            val last = visited.removeLast()
            stepSet.remove(last)
            currentDirection = last.direction
            currentPoint = last.point
        }
    ...
    fun possibleNewObstructionPositions(): Set<Point2D<Int>> {
        ...
        val pathSoFar = mutableListOf<GuardStep>()
        val visited = hashMapOf<Point2D<Int>, Int>()
        ...
        for (position in guardPositions) {
            ...
            if (!gridClone.isOnGrid(nextPoint)
                ...
                visited.computeIfAbsent(position.point) { pathSoFar.lastIndex }
                continue
            }

            gridClone[nextPoint] = OBSTACLE_SYMBOL

            // If we are updating an already visited point, then we have to test right before the first
            // time the nextPoint was encountered or from the start; otherwise, we test from just before this
            // position was added.
            val newPositions = if (visited.contains(nextPoint)) {
                val index = visited[nextPoint]!!
                val path = pathSoFar.subList(0, index)
                getGuardPositions(gridClone, path)
            } else {
                getGuardPositions(gridClone, pathSoFar)
            }
