...
class GardenRegion private constructor(
  ...
  companion object {
      ...
      region.graph.vertices.forEach { vertex ->
        if (vertex.size() < SIDES_PER_PLOT) {
          Direction.CARDINAL_DIRECTIONS.forEach { direction ->
            if (region.graph[vertex.get().point.pointInDirection(direction).id()]?.get() == null) {
              val neighbors = direction.perpendicular.mapNotNull { perpendicular ->
                val neighborPlot = region.graph[vertex.get().point.pointInDirection(perpendicular).id()]?.get()
                if (neighborPlot != null) {
                  if (region.graph[neighborPlot.point.pointInDirection(direction).id()]?.get() == null) {
                    perpendicular
                  } else {
                    null
                  }
    ...
    private fun fillSide(
      ...
      var last = vertex
      var next = graph.getVertex(last.get().point.pointInDirection(rightDirection).id()).getOrNull()

      while (next != null) {
        val nextPlot = next.get()
        val nextPlotInDirection = graph[nextPlot.point.pointInDirection(direction).id()]

        if (nextPlotInDirection == null) {
          side.add(next)
          last = next
          next = graph.get(last.get().point.pointInDirection(rightDirection).id())
        } else {
          next = null
        }
      }
    ...
    private fun findStart(
      ...
      var last = vertex
      var next = graph[last.get().point.pointInDirection(leftDirection).id()]
      while (next != null) {
        val nextPlot = next.get()
        val nextPlotInDirection = graph[nextPlot.point.pointInDirection(direction).id()]
        ...
        if (nextPlotInDirection == null) {
          last = next
          next = graph.getVertex(last.get().point.pointInDirection(leftDirection).id()).getOrNull()
        } else {
          next = null
        }
      }
